...
const util = require('../util/tokens')
const jwt = require('jsonwebtoken')
const sentMail = require('../middleware/nodeMailer')
const exp = require('express-validator')

/*register
 * @description:To validate the user inputs using express validator and  send the request body to service controller.
 * @param:request and response.
 **/
exports.register = (req, res) => {
    console.log("inside register");
    req.checkBody('username', 'username is not valid').isLength({ min: 3 }).isAlpha();
    ...
    if (errors) {
        ...
        userService.register1(req.body, (err, result) => {
            if (err) {
                console.log(err);
...
/**login
* @description:Take the user req and validate the email-id and password and send the request body to service controller.
*@param:request and response.
*/
        ...
        if (errors) {
            ...
            userService.login(req.body, (err, result) => {
                if (err) {
                    ...
                    var token = jwt.sign({ email: req.body.email, id: result[0]._id }, secret, { expiresIn: 864000 });
                    return res.status(200).send({
                        message: result,
                        "token": token
                    });
